...
public class AuthController {
    ...
    public static class LoginRequest { public String username; public String passwort; }

    @PostMapping("/login")
    public ResponseEntity<?> login(@RequestBody LoginRequest req) {
        if (req == null || req.username == null || req.passwort == null) return ResponseEntity.badRequest().build();
        ...
        User u = maybe.get();
        if (!u.getPassword().equals(req.passwort)) return ResponseEntity.status(401).body(Map.of("error","invalid credentials"));
    ...
    // helper used by other controllers to resolve current user from token (not strictly used everywhere)
